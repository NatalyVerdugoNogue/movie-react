    ...
    return (
      <Row>
        ...
        <form onSubmit={this.handleSubmit} >
          ...
          <Col l={6}>
            <Input className="desde-hasta-ranking" l={12} type='select' label="Desde" defaultValue='1'
              value={this.state.valueGte} onChange={this.handleChangeGte}>
              {score.map(score => <option value={score}>{score}</option>)}
            </Input>
          </Col>
          <Col l={6}>
            <Input className="desde-hasta-ranking" l={12} type='select' label="Hasta" defaultValue='10'
              value={this.state.valueLte} onChange={this.handleChangeLte}>
              {score.map(score => <option value={score}>{score}</option>)}
            </Input>
          </Col>
          <Col l={12} className="border-filtrar-ranking" >
            ...
            <Link to={`/ranking/${this.state.valueGte}/${this.state.valueLte}`}>
              <Button type='submit' className="filtrar-ranking">
                Filtrar
              </Button>
            </Link>
